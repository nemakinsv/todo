...
class ChangeOrderStatusWithYandexCheckoutForMarketplaces
{
    ...
    protected function sendReceipt(YandexCheckoutForMarketplaces $payment_processor, $payment_mode)
    {
        ...
        try {
            ...
            if (
                !empty($company_data['yandex_checkout_agent_type'])
                && AgentType::valueExists($company_data['yandex_checkout_agent_type'])
                && !empty($company_data['tax_number'])
            ) {
                $order_info['yandex_checkout_agent_type'] = $company_data['yandex_checkout_agent_type'];
