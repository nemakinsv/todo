...
class OrderDetailsBuilder
{
    ...
    public function buildOrderPlaces(array $order_info, array $shipment_info, array $auth = [])
    {
        ...
        if ($product_groups['package_info']['W'] === '0.001') {
            $weight = $service_params['weight'];
        } else {
    ...
    public function createPutDeliveryOptionsRequest(array $order_info, array $data)
    {
        ...
        $shipment = [
            'date' => isset($data['date']) ? date('Y-m-d', fn_parse_date($data['date'])) : date('Y-m-d', fn_parse_date(TIME + SECONDS_IN_DAY)),
        ...
        if (!empty($places)) {
            ...
            $this->shipping_service->isProductDimensionsExist(
                [
                    $service_params['width'],
                    $service_params['height'],
                    $service_params['length'],
                    $service_params['weight']
                ]
            )
        ) {
            $params['dimensions'] = [
